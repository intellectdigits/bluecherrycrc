...
app.post('/updateusers', urlencodedParser, function (req, res) {
  const sqlite3 = require('sqlite3').verbose();
 let datas = [req.body.username,req.body.email, req.body.password, req.body.exams, req.body.result, req.body.users, req.body.id];
// open database in memory
let db = new sqlite3.Database('./db/chinook.db', (err) => {
 if (err) {
   return console.error(err.message);
...
db.serialize(() => {
 // Queries scheduled here will be serialized.
 db.run(`UPDATE users SET username=?,email=?, password=?,exams=?,result=?,users=? WHERE id=?`, datas, function(err) {
     if (err) {
       return console.log(err.message);
     }
     // get the last insert id
...
app.post('/registerUser', urlencodedParser, function (req, res) {
  const sqlite3 = require('sqlite3').verbose();
 let datas = [crypto.randomUUID(), req.body.username,req.body.email, req.body.password, req.body.exams, req.body.result, req.body.users, req.body.id];
// open database in memory
let db = new sqlite3.Database('./db/chinook.db', (err) => {
 if (err) {
   return console.error(err.message);
...
db.serialize(() => {
 // Queries scheduled here will be serialized.
 db.run(`insert into users(id,username,email,password,result,exams,users,school)values(?,?,?,?,?,?,?,?)`, datas, function(err) {
  if (err) {
    return console.error(err.message);
  }
...
app.post('/addcat', urlencodedParser, function (req, res) {
  ...
  let db = new sqlite3.Database('./db/chinook.db', (err) => {
   ...
   db.serialize(() => {
    // Queries scheduled here will be serialized.
    let sql = `insert into cat(id,stud_name,course_code,course_title,text1,text2,school)values(?,?,?,?,?,?,?)`;
   ...
   // first row only
   db.run(sql, [crypto.randomUUID(),stud_name,course_code, course_title,text1,text2,school], (err, row) => {
     if (err) {
       return console.error(err.message);
     }else{
      res.render('cat');
